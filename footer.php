    ...
    <footer style="background: var(--dark-footer); color: #fbcfe8; font-size: 13px; padding: 50px 15px 0 15px; border-top: 1px solid #831843;">
        <div class="container">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 35px; padding-bottom: 40px;">
                ...
                <div>
                    ...
                    <ul style="list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 10px;">
                        <li><a href="<?php echo BASE_URL; ?>services.php" style="color: #fbcfe8; text-decoration: none; transition: color 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"><i class="fa fa-chevron-right" style="font-size: 10px; color: var(--secondary-gold);"></i> Verified Profile Search</a></li>
                        <li><a href="<?php echo BASE_URL; ?>services.php" style="color: #fbcfe8; text-decoration: none; transition: color 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"><i class="fa fa-chevron-right" style="font-size: 10px; color: var(--secondary-gold);"></i> Contact Privacy</a></li>
                        <li><a href="<?php echo BASE_URL; ?>services.php" style="color: #fbcfe8; text-decoration: none; transition: color 0.2s ease; display: inline-flex; align-items: center; gap: 6px;"><i class="fa fa-chevron-right" style="font-size: 10px; color: var(--secondary-gold);"></i> Biodata Assistance</a></li>
        ...
        $is_reg = ($curr_page === 'register.php');
        ?>
        <!-- Mobile Floating Sticky Action Bar (Dual Rectangular Buttons) -->
        <div class="mobile-floating-bar">
            <div style="display: flex; gap: 10px; width: 100%;">
                <a href="<?php echo BASE_URL; ?>register.php" class="floating-rect-btn btn-red-rect">
                    <i class="fa fa-user-plus"></i> Submit Biodata
                </a>
                <a href="<?php echo BASE_URL; ?>search.php" class="floating-rect-btn btn-dark-rect">
                    <i class="fa fa-search"></i> View Profiles
                </a>
            </div>
        </div>

        <!-- Floating WhatsApp CTA Button -->
        <a href="https://wa.me/918528600100?text=Hello%20Sainmatrimony,%20I%20want%20to%20inquire%20about%20matrimonial%20profiles" target="_blank" class="whatsapp-float-btn" title="Chat on WhatsApp">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var closeMobileDrawer = document.getElementById('closeMobileDrawer');

        function openMobileDrawer() {
            if (mobileDrawer) mobileDrawer.classList.add('drawer-open');
            if (mobileDrawerOverlay) mobileDrawerOverlay.classList.add('overlay-open');
            document.body.style.overflow = 'hidden';
        }

        function closeMobileDrawerFn() {
            if (mobileDrawer) mobileDrawer.classList.remove('drawer-open');
            if (mobileDrawerOverlay) mobileDrawerOverlay.classList.remove('overlay-open');
        ...
        if (closeMobileDrawer) {
            closeMobileDrawer.addEventListener('click', closeMobileDrawerFn);
        }
        if (mobileDrawerOverlay) {
            mobileDrawerOverlay.addEventListener('click', closeMobileDrawerFn);
        }
